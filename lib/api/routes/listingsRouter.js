...
export default async function listingsPlugin(fastify) {
  fastify.get('/table', async (request) => {
    ...
    const normalizedViewed = toBool(viewedFilter);
  ...
  fastify.post('/:listingId/viewed', async (request, reply) => {
    const { listingId } = request.params || {};
    const userId = request.session?.currentUser;
    if (!listingId || !userId) {
      return reply.code(400).send({ message: 'listingId or user not provided' });
    ...
    try {
      const changes = listingStorage.markListingViewed(listingId);
      if (changes === 0) {
        return reply.code(404).send({ message: 'Listing not found' });
      }
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ message: 'Failed to mark listing as viewed' });
    }
    return reply.send();
  });
