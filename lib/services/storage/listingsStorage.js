...
import { nullOrEmpty, fromJson, toJson } from '../../utils.js';
...
export const storeListings = (jobId, providerId, listings) => {
  ...
  SqliteConnection.withTransaction((db) => {
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, title, image_url, images, description, attributes, address,
                             link, created_at, is_active, latitude, longitude)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @rooms, @title, @image_url, @images, @description, @attributes, @address, @link,
    ...
    for (const item of listings) {
      const params = {
        ...
        images: Array.isArray(item.images) && item.images.length > 0 ? toJson(item.images) : null,
        description: item.description,
        attributes: Array.isArray(item.attributes) && item.attributes.length > 0 ? toJson(item.attributes) : null,
 ...
 * @param {boolean} [params.viewedFilter] - When true, only viewed listings; when false, only unviewed.
...
export const queryListings = ({
  ...
  // viewedFilter: when true -> only viewed listings, false -> only unviewed
  if (viewedFilter === true) {
    whereParts.push('(l.viewed = 1)');
  } else if (viewedFilter === false) {
    whereParts.push('(l.viewed = 0)');
  }
...
export const getListingById = (id, userId = null, isAdmin = false) => {
  ...
  // Only the detail view needs the gallery and structured attributes, so the JSON
  // arrays are parsed only here.
  if (row && typeof row.images === 'string') {
    row.images = fromJson(row.images, []);
  }
  if (row && typeof row.attributes === 'string') {
    row.attributes = fromJson(row.attributes, []);
  }
  return row;
};

/**
 * Mark a single listing as viewed.
 *
 * @param {string} id - The listing ID.
 * @returns {number} Number of rows affected (0 if listing not found).
 */
export const markListingViewed = (id) => {
  if (!id) return 0;
  const res = SqliteConnection.execute(`UPDATE listings SET viewed = 1 WHERE id = @id`, { id });
