...
// Nominatim `countrycodes` filter: Germany plus Austria (Willhaben, Austrian home addresses).
const COUNTRY_CODES = 'de,at';
...
async function doGeocode(address) {
  ...
  const url = `${API_URL}?q=${encodeURIComponent(address)}&format=json&countrycodes=${COUNTRY_CODES}`;
...
async function doAutocomplete(query) {
  ...
  const url = `${API_URL}?q=${encodeURIComponent(query)}&format=json&addressdetails=1&limit=5&countrycodes=${COUNTRY_CODES}`;
